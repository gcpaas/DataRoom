...
<script>
import {getUUID, msgEncode} from '@gcpaas/data-room-ui/packages/js/utils'
...
export default {
  ...
  methods: {
    ...
    getAccount(){
      const imgUrl = '/static/images/QQ.png' // 先获取图片路径
      this.$alert(`
    <div>
  ...
  `, '提示', {
    ...
    loginResponseHandler (res) {
      this.loading = false
      if (res.token) {
        tokenCacheService.set(res.token)
        let welcome = this.starter.login.welcome
        this.$notify({
          title: welcome?.title ?? '欢迎',
          message: welcome?.template ?? '您好，欢迎访问DataRoom设计器',
          type: 'success'
        })
        // 跳转到首页
