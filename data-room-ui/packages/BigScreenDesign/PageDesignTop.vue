<template>
  <div class="page-top-setting-wrap">
    ...
    <div class="head-btn-group">
      ...
      <el-dropdown trigger="click" class="align-list-dropdown">
        <CusBtn type="primary">
          对齐方式<i class="el-icon-arrow-down el-icon--right" />
        </CusBtn>
        <el-dropdown-menu slot="dropdown" class="align-dropdown-menu">
          <el-dropdown-item
            v-for="(mode, index) in alignList"
            :key="mode.value"
            @click.native="setAlign(mode.value)"
          >
            <icon-svg style="padding: 3px 8px" :name="iconList[index]" />
      ...
      <CusBtn @click.native="showHostory"> 历史操作 </CusBtn>
      <CusBtn :disabled="undoDisabled" @click.native="undo(true)">
        <i class="iconfont-bigscreen icon-jiantouqianjin icon-reverse" />
      </CusBtn>
      <CusBtn :disabled="redoDisabled" @click.native="undo(false)">
...
import { showSize, compressImage } from "data-room-ui/js/utils/compressImg";
...
export default {
  ...
  mounted() {
    ...
    this.$refs.zoomInput.$el.addEventListener(
      "mousewheel",
      this.handleMouseWheel,
    );
  },
  beforeDestroy() {
    this.$refs.zoomInput.$el.removeEventListener(
      "mousewheel",
      this.handleMouseWheel,
    );
  },
  methods: {
    ...
    setAlign(command) {
      ...
      // 找到选中组件内的xy最大最小值
      const maxXW = Math.max.apply(
        Math,
        activeChartList.map((item) => {
          return item.x + item.w;
        }),
      );
      const maxYH = Math.max.apply(
        Math,
        activeChartList.map((item) => {
      ...
      switch (command) {
        ...
        case "levelAround":
          ...
          // eslint-disable-next-line no-case-declarations
          const padding =
            (maxX - minXW - totalW) / (activeChartList.length - 1);
          // eslint-disable-next-line no-case-declarations
          let useW = 0;
          for (let i = 1; i < activeChartList.length - 1; i++) {
        ...
        case "verticalAround":
          ...
          // eslint-disable-next-line no-case-declarations
          const paddingBottom =
            (maxY - minYH - totalH) / (activeChartList.length - 1);
          // eslint-disable-next-line no-case-declarations
          let useH = 0;
          for (let i = 1; i < activeChartList.length - 1; i++) {
    ...
    backManagement() {
      ...
      this.$router.push({
        path:
          this.pageInfo.type === "component"
            ? window.BS_CONFIG?.routers?.componentUrl ||
    ...
    validateTabs(chartList) {
      ...
      if (chartList.length) {
        for (const chart of chartList) {
          if (
            chart.type === "chartTab" &&
            chart.customize.tabList.length !== 0
          ) {
      ...
      try {
        ...
        if (type === "preview") {
          ...
          try {
            ...
            // 确认框
            this.$confirm(
              "保存封面失败，我们将使用上次保存的封面，不会影响大屏数据的保存。可能是因为图片、视频资源跨域了导致使用toDataURL API生成图片失败，我们可以将资源上传到资源库。然后在组件中使用资源库中的图片资源，以确保没有跨域问题。",
              "提示",
              {
          ...
          if (dataUrl) {
            if (showSize(dataUrl) > 200) {
              ...
              const compressCoverPicture = await compressImage(dataUrl, {
                width: 1280,
                height: 720,
                size: 400,
                quality: 1,
        ...
        .catch((error) => {
          ...
          if (error.type === "error") {
            ...
            if (error.currentTarget.tagName.toLowerCase() === "img") {
              // 确认框
              this.$confirm(
                "图片资源跨域导致使用toDataURL API生成图片失败，请将图片上传到资源库，然后在组件中使用资源库中的图片资源，确保没有跨域问题。",
                "提示",
                {
                  confirmButtonText: "确定",
                  showCancelButton: false,
