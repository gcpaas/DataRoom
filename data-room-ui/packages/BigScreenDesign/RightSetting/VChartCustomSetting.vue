    ...
    >
      ...
      <div class="lc-field-body">
        <el-form-item label="标题" label-width="120px">
          <el-input v-model="config.title" placeholder="请输入标题" clearable />
        </el-form-item>
      </div>
      ...
      <div class="lc-field-body">
        <PosWhSetting label-width="120px" :config="config" />
      </div>
...
import VChartColorSelect from "data-room-ui/VChartColorMultipleSelect/index.vue";
...
export default {
  ...
  watch: {
    groupList: {
      ...
      handler(val) {
        ...
        this.$store.commit("bigScreen/changeActiveItemConfig", {
          ...this.config,
          setting: newArr,
        });
      },
  ...
  mounted() {
    ...
    const groupNameList = [];
    this.config.setting
      .filter((item) => item.tabName === "custom")
      .forEach((item) => {
        if (item.tabName === "custom" && item.groupName) {
          if (!groupNameList.includes(item.groupName)) {
            ...
            this.groupList
              .find((group) => group.groupName === item.groupName)
              .list.push(item);
          }
        } else {
          if (this.groupList.find((group) => group.groupName === "other")) {
            this.groupList
              .find((group) => group.groupName === "other")
              .list.push(item);
          } else {
            this.groupList.push({
              groupName: "other",
