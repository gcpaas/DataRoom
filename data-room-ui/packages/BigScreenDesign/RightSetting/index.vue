  ...
  <div class="bs-setting-wrap" @click.stop>
    ...
    >
      <el-tab-pane label="数据" name="data">
        <DataSetting ref="dataSetting" :key="config.code">
    ...
    <el-scrollbar v-else class="bs-setting-wrap bs-scrollbar">
      <component
        :is="resolveComponentType(config.type)"
        ref="customSetting"
        :key="config.code"
...
export default {
  ...
  components: {
    ...
    VChartComponent: VChartCustomSetting,
  },
  data() {
    return {
      activeName: "data",
  ...
  computed: {
    ...
    configDataSource() {
      return {
        ...
        dataSourceSetting:
          cloneDeep(
            this.config?.setting?.filter((item) => item.tabName === "data"),
          ) || [],
  ...
  methods: {
    ...mapMutations("bigScreen", ["saveTimeLine"]),
    debounce(delay, obj) {
      if (this.timeout) {
