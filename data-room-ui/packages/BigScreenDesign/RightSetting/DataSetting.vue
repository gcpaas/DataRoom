      ...
      <div class="data-setting-box">
        ...
        >
          <div class="lc-field-head">
            <div class="lc-field-title">数据来源</div>
          </div>
          <div class="lc-field-body">
            <el-form-item
              v-if="
                config.option.displayOption.expression &&
                config.option.displayOption.expression.enable
              "
              label="数据来源"
            >
              <el-select
            ...
            <el-form-item
              v-if="
                config.dataSource.source === 'expression' &&
                config.option.displayOption.expression &&
                config.option.displayOption.expression.enable
              "
              label="表达式"
        ...
        >
          <div class="lc-field-head">
            <div class="lc-field-title">数据处理</div>
          </div>
          <div class="lc-field-body">
        ...
        <div
          v-if="
            !['tree', 'multipleNumberChart', 'carousel'].includes(
              config.type,
            ) &&
            config.option.displayOption.dataSourceType.enable &&
            !['expression', 'static'].includes(config.dataSource.source)
          "
          class="data-setting-data-box"
        >
          <div class="lc-field-head">
            <div class="lc-field-title">数据配置</div>
          </div>
          <!--  基础组件数据配置  -->
          <template
            v-if="
          ...
          >
            ...
            <el-form-item
              ...
              :prop="
                config.option.displayOption.dimensionField.multiple
        ...
        <div
          v-if="
            config.option.displayOption.headerField &&
            config.option.displayOption.headerField.enable
          "
          name="轮播表配置"
        >
          <template slot="title">
            <div class="lc-field-head">
              <div class="lc-field-title">轮播表配置</div>
            </div>
          </template>
          <div class="lc-field-body">
            <el-table
        ...
        <div
          v-if="
            config.option.displayOption.mapField &&
            config.option.displayOption.mapField.enable
          "
          name="地图数据配置"
        >
          <template slot="title">
            <div class="lc-field-head">
              <div class="lc-field-title">地图数据配置</div>
            </div>
          </template>
          <div class="lc-field-body">
        ...
        <div
          v-if="
            config.option.displayOption.flyMapField &&
            config.option.displayOption.flyMapField.enable
          "
          name="飞线地图数据配置"
        >
          <div class="lc-field-body">
        ...
        <div
          v-if="
            config.option.displayOption.params.enable &&
            !['expression', 'static'].includes(config.dataSource.source)
          "
          class="data-setting-data-box"
        >
          <div class="lc-field-head">
            <div class="lc-field-title">参数配置</div>
          </div>
          <div class="lc-field-body">
            <el-table
              ref="singleTable"
        ...
        <ComponentRelation
          v-if="
            !['carousel', 'gauge', 'liquid', 'marquee'].includes(config.type)
          "
          :config="config"
          :source-field-list="sourceFieldList"
        />
...
export default {
  ...
  computed: {
    ...
    sourceFieldList() {
      ...
      if (
        ["input", "timePicker", "dateTimePicker"].includes(this.config.type)
      ) {
      ...
      if (["select"].includes(this.config.type)) {
        if (this.config.customize?.placeholder) {
          this.config.customize.placeholder =
            "请选择" +
            this.dataSourceDataList.find((item) => item.fieldName === val)
              .comment;
        }
      }
    },
  ...
  mounted() {
    ...
    if (
      this.config.option.displayOption.flyMapField &&
      this.config.option.displayOption.flyMapField.enable
    ) {
    ...
    getDataSetDetailsById(id, type) {
      if (id) {
        ...
        getDataSetDetails(id).then((res) => {
          ...
          this.config.option.displayOption.params.enable =
            res.type !== "dataModel";
