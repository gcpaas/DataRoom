...
import {
  settingToTheme,
  themeToSetting,
} from "data-room-ui/js/utils/themeFormatting";
    ...
    return new Promise((resolve) => {
      getScreenInfo(code).then((data) => {
        ...
        pageInfo.chartList.forEach((chart) => {
            ...
            for (const [key, localPlotSetting] of plotSettings) {
              if (chart.name == localPlotSetting.name) {
                ...
                const localSettings = JSON.parse(
                  JSON.stringify(localPlotSetting.setting),
                );
                chart.setting = localSettings.map((localSet) => {
                  // 在远程组件配置中找到 与 本地组件的配置项 相同的项索引
                  const index = chart.setting.findIndex(
                    (remoteSet) => remoteSet.field == localSet.field,
                  );
                  if (index !== -1) {
                    // 使用远程的值替换本地值
          ...
          } else if (chart.type === "vChartComponent") {
            // 为本地echarts组件配置添加迭代器
            if (!echartSettingsIterator) {
              echartSettings[Symbol.iterator] = function* () {
                const keys = Object.keys(echartSettings);
        ...
        pageInfo.pageConfig.cacheDataSets?.map((cacheDataSet) => {
          dispatch("getCacheDataSetData", {
            dataSetId: cacheDataSet.dataSetId,
          });
...
export function handleResData(data) {
  ...
  pageInfo.chartList.forEach((chart) => {
    if (
      !["customComponent", "remoteComponent", "echartsComponent"].includes(
        chart.type,
      )
    ) {
      originalConfig = {
        option: { ...setModules[chart.type] },
        ...dataModules[chart.type],
      };
      ...
      if (!chart?.dataSource?.businessKey) {
        chart.option.data =
          plotList?.find((plot) => plot.name === chart.name)?.option?.data ||
          chart?.option?.data;
      }
      // 如果没有版本号，或者版本号修改了则需要进行旧数据兼容
      if (
        !chart.version ||
        (originalConfig && chart.version !== originalConfig?.version)
      ) {
        // TODO 远程组件需要重新写处理函数
        if (chart.type === "customComponent") {
          chart = compatibility(chart, originalConfig);
...
function compatibility(config, originalConfig) {
  ...
  newConfig.dataSource = objCompare(
    newConfig?.dataSource,
    originalConfig?.dataSource,
  );
  newConfig.customize = objCompare(
    newConfig?.customize,
    originalConfig?.customize,
  );
  newConfig.option = {
...
function objCompare(obj1, obj2) {
  ...
  // 交集
  const intersection =
    keys1?.filter(function (v) {
      return keys2.indexOf(v) > -1;
    }) || [];
  // 差集
  const differenceSet =
    keys2?.filter(function (v) {
