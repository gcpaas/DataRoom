...
import { setDataRoomTheme } from "./chartTheme";
...
export default {
  ...
  methods: {
     ...
     */
    newChart(config) {
      console.log("newVChart", config.option);
      this.chart = new VChart(config.option, {
        dom: this.chatId,
      });
        ...
        if (set.optionField) {
          ...
          optionField.forEach((field, index) => {
            if (index === optionField.length - 1) {
              // 数据配置时，必须有值才更新
              if (
                (set.tabName === type && type === "data" && set.value) ||
                (set.tabName === type && type === "custom")
              ) {
    ...
    dataFormatting(config, data) {
      ...
      if (data.success) {
        ...
        if (config.chartType == "Treemap") {
          const xAxis = config.setting.find(
            (item) => item.field === "xField",
          )?.value;
          ...
          const yAxis = config.setting.find(
            (item) => item.field === "yField",
          )?.value;
      ...
      //   // 数据返回失败则赋前端的模拟数据
      //   config.option.data =
      //     this.plotList?.find((plot) => plot.name === config.name)?.option
      //       ?.data || config?.option?.data;
      //   const _xField =
      //     this.plotList?.find((plot) => plot.name === config.name)?.option
      //       ?.xField || config?.option?.xField;
      //   const _yField =
      //     this.plotList?.find((plot) => plot.name === config.name)?.option
      //       ?.yField || config?.option?.yField;
      //   const _seriesField =
      //     this.plotList?.find((plot) => plot.name === config.name)?.option
      //       ?.seriesField || config?.option?.seriesField;
      //   config.option = _seriesField
      //     ? {
      //         ...config.option,
      //         xField: _xField,
      //         yField: _yField,
      ...
      //       }
      //     : { ...config.option, xField: _xField, yField: _yField };
      // }
      return config;
    },
    ...
    changeStyle(config, isUpdateTheme) {
      ...
      if (this.chart) {
        console.log("changeStyle", config.option);
        this.chart.updateSpec(config.option);
      }
      return config;
    },
  },
