...
$frontBuildDir = Join-Path $projectRoot "dataRoomFront/front"
$scriptSourceDir = Join-Path $projectRoot "doc/script"
...
[xml]$pom = Get-Content -Path (Join-Path $projectRoot "pom.xml")
...
Write-Host "准备打包前端"
Push-Location (Join-Path $projectRoot "dataRoomFront")
try {
...
Copy-Item -Path (Join-Path $projectRoot "dataRoomServer/target/dataRoomServer.jar") -Destination $packageDir
...
Copy-Item -Path (Join-Path $projectRoot "dataRoomServer/src/main/resources/*.yml") -Destination $packageConfigDir
