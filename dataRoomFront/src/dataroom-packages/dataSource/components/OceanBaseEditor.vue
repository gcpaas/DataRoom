...
const formData = reactive<DataSourceEntity>({
  ...
  dataSource: {
    dataSourceType: 'oceanbase',
    driverName: 'com.oceanbase.jdbc.Driver',
    username: '',
    password: '',
    url: '',
  },
  ...
  (newVal) => {
    if (newVal) {
      ...
      if (!formData.dataSource.driverName) {
        formData.dataSource.driverName = 'com.oceanbase.jdbc.Driver'
      }
    }
  },
...
<template>
  <el-form class="data-source-editor-form" ref="formRef" :model="formData" :rules="rules" label-width="100px">
    ...
    <el-form-item label="连接地址" prop="dataSource.url">
      <el-input v-model="formData.dataSource.url" type="textarea" :rows="3" placeholder="jdbc:oceanbase://localhost:2881/test" clearable />
    </el-form-item>
  </el-form>
</template>
