  ...
  (newVal) => {
    if (newVal) {
      ...
      if (!formData.dataSource.driverName) {
        formData.dataSource.driverName = 'com.mongodb.jdbc.MongoDriver'
      }
    }
  },
...
<template>
  <el-form class="data-source-editor-form" ref="formRef" :model="formData" :rules="rules" label-width="100px">
    ...
    <el-form-item label="连接地址" prop="dataSource.url">
      <el-input v-model="formData.dataSource.url" type="textarea" :rows="3" placeholder="jdbc:mongodb://localhost:27017/?database=test" clearable />
    </el-form-item>
  </el-form>
</template>
