...
const formData = reactive<DataSourceEntity>({
  name: '',
  dataSourceType: 'tdengine',
  dataSource: {
    dataSourceType: 'tdengine',
    driverName: 'com.taosdata.jdbc.ws.WebSocketDriver',
    username: '',
    password: '',
    url: '',
  },
  ...
  (newVal) => {
    if (newVal) {
      ...
      if (!formData.dataSource.driverName) {
        formData.dataSource.driverName = 'com.taosdata.jdbc.ws.WebSocketDriver'
      }
    }
  },
...
<template>
  <el-form class="data-source-editor-form" ref="formRef" :model="formData" :rules="rules" label-width="100px">
    ...
    <el-form-item label="连接地址" prop="dataSource.url">
      <el-input v-model="formData.dataSource.url" type="textarea" :rows="3" placeholder="jdbc:TAOS-WS://localhost:6041/test?varcharAsString=true" clearable />
    </el-form-item>
  </el-form>
</template>
