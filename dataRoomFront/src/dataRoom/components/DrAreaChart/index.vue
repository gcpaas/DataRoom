...
import {mockDataset, type DrAreaChartConfig} from './install.ts'
  ...
  if (seriesFieldName && data.some((item: any) => item[seriesFieldName])) {
    ...
    seriesList = seriesNames.map((sName, idx) => {
      const seriesData = categories.map(cat => {
        ...
        return item ? Number(item[yFieldName]) || 0 : 0
      })
 ...
 * 构建单个系列配置
 */
const buildSeriesItem = (name: string, data: number[], index: number): echarts.LineSeriesOption => {
  const props = chart.props
  ...
  // 区域填充样式
  let areaStyle: any
  if (props.series.areaStyle.gradient.enabled) {
    areaStyle = {
      opacity: props.series.areaStyle.opacity,
      color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
        {offset: 0, color: adjustColorOpacity(baseColor, props.series.areaStyle.gradient.topOpacity)},
        {offset: 1, color: adjustColorOpacity(baseColor, props.series.areaStyle.gradient.bottomOpacity)}
      ])
    }
  } else {
    areaStyle = {
  ...
  return {
    ...
    smooth: props.series.lineType === 'smooth',
    showSymbol: props.series.symbol.show,
    symbol: props.series.symbol.type,
...
const initChart = () => {
  ...
  chartInstance.on('click', (params: any) => {
    ...
    // 同时触发区域单击事件
    canvasInst.triggerChartBehavior(chart.id, 'areaClick', {
      seriesName: params.seriesName
    })
  })
...
<template>
  <div class="dr-area-chart" :id="chart.id" ref="chartRef"></div>
</template>
