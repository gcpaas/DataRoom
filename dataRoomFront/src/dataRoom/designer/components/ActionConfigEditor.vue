...
const normalizeActions = () => {
  props.actions.forEach((action, index) => ensureActionConfig(action, index))
}

const currentAction = computed(() => {
  if (activeActionIndex.value >= 0 && activeActionIndex.value < props.actions.length) {
    return props.actions[activeActionIndex.value]
  }
  return null
})
...
const addAction = () => {
  const newAction = createDefaultChartAction(props.actions.length + 1)
  props.actions.push(newAction)
  activeActionIndex.value = props.actions.length - 1
...
const deleteAction = (index: number) => {
  ...
  if (activeActionIndex.value >= props.actions.length) {
    activeActionIndex.value = Math.max(0, props.actions.length - 1)
  }
...
const handleDrop = (e: DragEvent, dropIndex: number) => {
  ...
  if (dragStartIndex.value !== -1 && dragStartIndex.value !== dropIndex) {
    const dragItem = props.actions[dragStartIndex.value]
    if (!dragItem) {
      return
    }
    props.actions.splice(dragStartIndex.value, 1)
    props.actions.splice(dropIndex, 0, dragItem)
...
<template>
  <div class="dr-action-config-editor">
    <div class="action-list-wrapper">
      ...
      <el-scrollbar>
        ...
        <div v-if="actions.length === 0" class="empty-action-list">
          <el-empty description="暂无动作，请点击上方按钮添加" :image-size="80" />
        </div>
      </el-scrollbar>
    </div>
