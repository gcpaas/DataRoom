...
- `dataRoomFront/src/dataRoom/hooks/use-canvas-inst/index.ts` 已有 `fillDatasetParams`、`triggerChartBehavior`、`updateGlobalVariableValue`。
- `dataRoomFront/src/dataRoom/_components/BehaviorConfigDialog.vue` 已有行为动作配置界面。
- `dataRoomFront/src/dataRoom/components/type/ChartAction.ts` 当前动作结构较简单，主要支持 `code`。
...
- `dataRoomFront/src/dataRoom/hooks/use-timer-manager/index.ts` 已有定时器管理，但主要执行 code action。
- `dataRoomFront/src/dataRoom/hooks/use-dr-component/index.ts` 已有 `autoRefreshData`，组件挂载时会自动拉取数据。
